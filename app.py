...
agents_col = st.selectbox("Agents Staffed column (per-skill values)", cols, index=idx_or_default(cols, cfg_get("agents_col", agents_guess or cols[0])))
aht_col    = st.selectbox("Average Handle Time (AHT) column", cols, index=idx_or_default(cols, cfg_get("aht_col",    aht_guess    or cols[0])))

# NOTE: label uses "Abandon %"
...
# ------------------------------
# 2) Secondary report (no-skill) for Total Agents
# ------------------------------
st.subheader("Optional: Secondary Report (no skill filter) — for Total Agents")
uploaded2 = st.file_uploader("Second report (CSV/XLSX/XLS) — overall totals / no skill filter", type=["csv", "xlsx", "xls"], key="second")

second_df = None
total_agents_from_second = None
agents2_col = None
agents2_guess = None
...
if uploaded2 is not None:
    ...
    if second_df is not None and not second_df.empty:
        st.dataframe(second_df.head(20), use_container_width=True)
        # Try to auto-detect agents column
        agents2_guess = find_col(second_df, AGENTS_SYNS)
        cols2 = list(second_df.columns)
        agents2_col = st.selectbox("Agents Staffed column (from 2nd report)", cols2, index=idx_or_default(cols2, agents2_guess or cols2[0]))
        # Aggregation choice
        agg_choice = st.selectbox("How to compute Total Agents from 2nd report?", ["Sum (default)", "Max"], index=0)
        agents2_num = pd.to_numeric(second_df[agents2_col], errors="coerce").fillna(0)
        total_agents_from_second = int(agents2_num.sum() if agg_choice.startswith("Sum") else agents2_num.max())
    elif second_df is not None and second_df.empty:
        st.warning("The second report appears to be empty.")
...
cfg = {
    ...
    # Optional persistence (only meaningful if 2nd report schema is stable)
    "agents2_col": agents2_col if agents2_col else "",
}
...
# ---- Defensive: verify required columns exist (main) ----
missing = [c for c in [skill_col, calls_col, agents_col, aht_col] if c not in df.columns]
if missing:
    st.error(f"Selected columns not found in main file: {missing}")
    st.stop()

# ---- Normalize & alias skills in the MAIN DATAFRAME ----
...
# Use 2nd report's Agents total if provided; otherwise sum per-skill
if total_agents_from_second is not None:
    total_agents = total_agents_from_second
